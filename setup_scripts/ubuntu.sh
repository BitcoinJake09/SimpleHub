...
if [ $(echo `pwd` | grep setup_scripts | wc -l) -ne 0 ]; then
	...
	# place spigot alongside simplehub
	mkdir -p ../spigot
	cd ../spigot
	spigot_dir=`pwd`
	rm $spigot_file 2> /dev/null
	wget http://jenkins.simplehub.co/job/spigot/lastSuccessfulBuild/artifact/Spigot/Spigot-Server/target/$spigot_file
	...
	echo 'Creating the plugins directory and linking the simplehub plugin'
	mkdir -p plugins
	rm plugins/simplehub-all.jar 2> /dev/null
	ln -s $simplehub_dir/build/libs/simplehub-all.jar $spigot_dir/plugins/simplehub-all.jar
